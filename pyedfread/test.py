...
class TestEdfread(unittest.TestCase):
    ...
    def test_messages(self):
        ...
        """
        self.assertTrue(all((self.messages["sample"].values) == np.arange(6, 292, 5)))
    ...
    def test_events(self):
        ...
        self.assertEqual(
            np.round(self.events.gavx.mean(), 3), np.round(420.842680343156, 3)
        )
        self.assertEqual(
            np.round(self.events.gavy.mean(), 3), np.round(205.31814411595926, 3)
        )
